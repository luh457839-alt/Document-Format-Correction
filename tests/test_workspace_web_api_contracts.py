...
class WebApiHttpRegressionTest(unittest.TestCase):
    def test_template_configs_scan_root_templates_json_files(self) -> None:
        ...
        (templates_dir / "contract.json").write_text(
            json.dumps(
                {
                    "template_meta": {
        ...
        (templates_dir / "fallback.json").write_text("{}", encoding="utf-8")
        ...
        self.assertEqual(templates[0]["id"], "contract-template")
        self.assertEqual(templates[0]["name"], "合同模板")
        self.assertEqual(templates[0]["version"], "1.2.0")
        self.assertEqual(templates[0]["description"], "合同格式模板")
        self.assertEqual(templates[0]["tags"], ["contract", "cn"])
        self.assertEqual(templates[0]["path"], str(templates_dir / "contract.json"))
        self.assertEqual(templates[1]["id"], "fallback")
        self.assertEqual(templates[1]["name"], "fallback")
        self.assertEqual(templates[1]["version"], "unknown")

    def test_workspace_document_upload_default_config_format_run_and_download(self) -> None:
        root = Path(".tmp") / f"web-api-workspace-{uuid4().hex}"
        front_dist_dir = root / "front"
        upload_dir = root / "uploads"
        templates_dir = root / "templates"
        ...
        (templates_dir / "template.json").write_text(
            json.dumps({"template_meta": {"id": "template-main", "name": "主模板", "version": "1.0.0"}}, ensure_ascii=False),
            encoding="utf-8",
        )
        docx_path = root / "input.docx"
        rejected_path = root / "input.txt"
        output_path = output_dir / "formatted.docx"
        docx_path.write_bytes(b"PK\x03\x04docx")
        rejected_path.write_text("nope", encoding="utf-8")
        expected_output_bytes = b"PK\x03\x04formatted"
        output_path.write_bytes(expected_output_bytes)

        try:
            with patch("src.python.gui.web_api.create_session", return_value={}), patch(
                "src.python.gui.web_api.attach_document", return_value={}
            ), patch(
                "src.python.gui.web_api.submit_agent_turn",
                return_value={
                    "response": {
                        "outputDocxPath": str(output_path),
                        "summary": "格式修改任务已完成",
                    }
                },
            ):
                ...
                server.start()
                try:
                    rejected = self._post_multipart(f"{server.base_url}/api/workspace/documents", rejected_path)
                    uploaded = self._post_multipart(f"{server.base_url}/api/workspace/documents", docx_path)
                    defaults = self._get_json(f"{server.base_url}/api/workspace/default-config")
                    document_id = uploaded["json"]["document"]["documentId"]
                    created = self._post_json(
                        f"{server.base_url}/api/workspace/format-runs",
                        {
                            "documentId": document_id,
                            "config": defaults["json"]["config"],
                            "commandText": "统一正文字体",
                            "customRequirement": "保留图片",
                            "templateId": "template-main",
                        },
                    )
                    job_id = created["json"]["job"]["jobId"]
                    time.sleep(0.2)
                    completed = self._get_json(f"{server.base_url}/api/workspace/format-runs/{job_id}")
                    history = self._get_json(f"{server.base_url}/api/workspace/history")
                    downloaded = self._get_bytes(f"{server.base_url}/api/workspace/format-runs/{job_id}/download")
                finally:
                    server.stop()
        finally:
            shutil.rmtree(root, ignore_errors=True)
        ...
        self.assertTrue(document["documentId"].startswith("doc-"))
        self.assertEqual(document["fileName"], "input.docx")
        self.assertEqual(document["analysis"]["health"]["status"], "unknown")
        self.assertEqual(document["analysis"]["outline"], [])
        self.assertEqual(defaults["status"], 200)
        self.assertEqual(defaults["json"]["config"]["globalTypography"]["fontFamily"]["mode"], "default")
        self.assertEqual(defaults["json"]["templates"][0]["id"], "template-main")
        self.assertEqual(created["status"], 202)
        self.assertEqual(
            [step["id"] for step in created["json"]["job"]["steps"]],
            ["validate_request", "assemble_prompt", "run_agent", "validate_output", "finalize"],
        )
        ...
        self.assertEqual(completed["json"]["job"]["downloadUrl"], f"/api/workspace/format-runs/{job_id}/download")
        self.assertEqual(history["json"]["runs"][0]["jobId"], job_id)
        self.assertEqual(downloaded["status"], 200)
        self.assertEqual(downloaded["body"], expected_output_bytes)
        self.assertEqual(
            downloaded["headers"].get("content-type"),
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        )

    def test_workspace_download_rejects_incomplete_jobs(self) -> None:
        root = Path(".tmp") / f"web-api-workspace-download-{uuid4().hex}"
        front_dist_dir = root / "front"
        upload_dir = root / "uploads"
        templates_dir = root / "templates"
        front_dist_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            with patch("src.python.gui.web_api.create_session", return_value={}), patch(
                "src.python.gui.web_api.attach_document", return_value={}
            ), patch("src.python.gui.web_api.submit_agent_turn", side_effect=RuntimeError("still running")):
                server = WebApiServer(
                    WebApiConfig(
                        port=0,
                        front_dist_dir=front_dist_dir,
                ...
                try:
                    ...
                    job_id = created["json"]["job"]["jobId"]
                    rejected = self._get_json(f"{server.base_url}/api/workspace/format-runs/{job_id}/download")
                finally:
                    server.stop()
        finally:
            shutil.rmtree(root, ignore_errors=True)
        ...
        self.assertEqual(rejected["json"]["error"]["code"], "E_FORMATTING_JOB_NOT_COMPLETED")
        self.assertEqual(rejected["json"]["error"]["jobId"], job_id)
    ...
    def test_template_runs_fail_when_output_path_is_missing_after_applied_result(self) -> None:
        root = Path(".tmp") / f"web-api-template-runs-missing-output-{uuid4().hex}"
        front_dist_dir = root / "front"
        upload_dir = root / "uploads"
        templates_dir = root / "templates"
        output_dir = root / "output"
        ...
        self.assertEqual(
            failed["json"]["job"]["summary"],
            "E_TEMPLATE_OUTPUT_NOT_FOUND: template run did not provide output_docx_path.",
        )
        self.assertEqual(
            failed["json"]["job"]["error"]["message"],
            "E_TEMPLATE_OUTPUT_NOT_FOUND: template run did not provide output_docx_path.",
        )
        self.assertEqual(
            [step["id"] for step in failed["json"]["job"]["steps"]],
            ["load_inputs", "run_template_pipeline", "validate_result", "materialize_output"],
        )
        self.assertEqual(
            next(step for step in failed["json"]["job"]["steps"] if step["id"] == "materialize_output")["status"],
            "failed",
        )

    def test_template_runs_fail_when_output_path_is_unreadable_after_applied_result(self) -> None:
        root = Path(".tmp") / f"web-api-template-runs-unreadable-{uuid4().hex}"
        front_dist_dir = root / "front"
        upload_dir = root / "uploads"
        templates_dir = root / "templates"
        output_dir = root / "output"
        ...
        document_path = upload_dir / "input.docx"
        template_path = templates_dir / "template.json"
        unreadable_output_path = output_dir / "formatted.docx"
        document_path.write_bytes(b"PK\x03\x04docx")
        template_path.write_text("{}", encoding="utf-8")
        unreadable_output_path.write_text("", encoding="utf-8")
        ...
        report = {
            ...
            "execution_result": {
                "applied": True,
                "output_docx_path": str(unreadable_output_path),
                "change_summary": "模板已套用到正文样式",
                "issues": [],
            },
        }
        ...
        self.assertEqual(
            failed["json"]["job"]["summary"],
            "E_TEMPLATE_OUTPUT_NOT_FOUND: output DOCX does not exist: " + str(unreadable_output_path),
        )
    ...
    def test_template_run_poll_returns_bridge_stage_failure_detail(self) -> None:
        ...
        self.assertIn(
            ...
            failed["json"]["job"]["error"]["details"]["stderrSummary"],
        )
        pipeline_step = next(
    ...
    def test_web_api_async_submit_requires_readable_output_before_completed(self) -> None:
        bridge_state = {
            "sessionId": "chat-main",
            "attachedDocument": None,
            "turns": [],
        }
        output_path = Path(".tmp") / f"web-api-chat-output-{uuid4().hex}" / "output.docx"
        ...
        def fake_submit_agent_turn(session_id: str, user_input: str, *, timeout_sec=None, options=None):  # noqa: ANN001
            ...
            return {
                "session": self._session_snapshot(bridge_state),
                "response": {"content": "处理完成", "outputDocxPath": str(output_path)},
            }
        ...
        def fake_get_turn_run_status(*, session_id=None, turn_run_id=None, timeout_sec=None, options=None):  # noqa: ANN001
            ...
            return {
                "turnRun": {
                    ...
                    "steps": [
                        {"id": "decide_mode", "title": "判定模式", "status": "completed"},
                        {"id": "generate_reply", "title": "生成回复", "status": "completed"},
                    ],
                    "createdAt": 1,
                    "updatedAt": 2,
                }
        ...
        self.assertEqual(completed["status"], 200)
        self.assertEqual(completed["json"]["job"]["status"], "failed")
        self.assertEqual(
            completed["json"]["job"]["summary"],
            f"E_OUTPUT_NOT_FOUND: output path does not exist: {output_path}",
        )
        self.assertNotIn("outputPath", completed["json"]["job"])
    ...
    def _get_bytes(self, url: str) -> dict[str, object]:
