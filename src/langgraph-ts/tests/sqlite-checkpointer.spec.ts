...
import { SqliteCheckpointer } from "../runtime/sqlite-checkpointer.js";

const tempDirs: string[] = [];

async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "langgraph-runtime-sqlite-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("phase 3 sqlite checkpointer", () => {
  ...
  it("round-trips agent state with message restoration and executed patch keys", async () => {
    ...
    const checkpointer = new SqliteCheckpointer(sqlitePath);
