...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "langgraph-mutation-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("phase 4 bundle mutation", () => {
  it("applies inline and paragraph style writes to xml and document truth", async () => {
    ...
    expect(documentXml).toMatch(/w:rFonts[^>]*w:eastAsia="Arial"/);
    expect(documentXml).toMatch(/w:sz[^>]*w:val="28"/);
    expect(documentXml).toMatch(/w:szCs[^>]*w:val="28"/);
