...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "langgraph-runtime-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("phase 3 runtime graph", () => {
  it("routes template payloads into template mode and applies the same write tool path", async () => {
    ...
    const templateTask: TemplateTaskPayload = {
  ...
  it("persists executed_patch_keys in checkpoint state and skips duplicate mutation on replay", async () => {
    ...
    const firstRun = await runDocumentAgentGraph({
    ...
    const secondRun = await runDocumentAgentGraph({
      thread_id: "checkpoint-thread",
      document_path: fixture.docxPath,
  ...
  it("passes projection intent into projection builder and reasoning context", async () => {
    ...
    const projectionIntent: ProjectionIntent = {
...
class StaticModelAdapter implements RuntimeModelAdapter {
  ...
  async invoke(messages: BaseMessage[], _input: RuntimeInput): Promise<AIMessage> {
...
class CapturingModelAdapter implements RuntimeModelAdapter {
...
class InMemoryCheckpointer implements RuntimeCheckpointer {
