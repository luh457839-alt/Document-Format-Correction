...
beforeAll(async () => {
  tempDir = await mkdtemp(path.join(os.tmpdir(), "langgraph-write-tool-"));
...
describe("phase 2 write target resolution", () => {
  ...
  it("treats paragraph ids in node_ids as writable paragraph targets", () => {
    ...
    const analysis = analyzeWriteTarget(bundle, {
      kind: "node_ids",
      node_ids: [firstWritableParagraph.id]
    });

    expect(analysis.missing_node_ids).toEqual([]);
    expect(analysis.matched_paragraph_ids).toEqual([firstWritableParagraph.id]);
    expect(analysis.target_node_ids).toEqual(firstWritableParagraph.runIds);
    expect(analysis.patch_target_ids).toEqual(firstWritableParagraph.runIds.map((runId) => `target:inline:${runId}`));
  });
});
