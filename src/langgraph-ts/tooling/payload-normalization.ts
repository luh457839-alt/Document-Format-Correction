  ...
  switch (operation) {
    ...
    case "set_size": {
      ...
      if (fontSize === undefined) {
        throw invalidPayload(operation, "set_size requires font_size_pt");
      }
    ...
    case "set_line_spacing": {
      ...
      if (lineSpacing === undefined) {
        throw invalidPayload(operation, "set_line_spacing requires a positive line_spacing");
      }
    ...
    case "set_bold":
      return { is_bold: pickRequiredBoolean(operation, "is_bold", payload.is_bold, payload.isBold) };
    case "set_italic":
      return { is_italic: pickRequiredBoolean(operation, "is_italic", payload.is_italic, payload.isItalic) };
    case "set_underline":
      return { is_underline: pickRequiredBoolean(operation, "is_underline", payload.is_underline, payload.isUnderline) };
    case "set_strike":
      return { is_strike: pickRequiredBoolean(operation, "is_strike", payload.is_strike, payload.isStrike) };
    case "set_highlight_color": {
      const highlight = pickHighlightColor(payload.highlight_color, payload.highlightColor);
      if (!highlight) {
        throw invalidPayload(operation, "set_highlight_color requires highlight_color");
      }
      return { highlight_color: highlight };
    }
    case "set_all_caps":
      return { is_all_caps: pickRequiredBoolean(operation, "is_all_caps", payload.is_all_caps, payload.isAllCaps) };
    case "set_page_layout": {
      ...
      if (Object.keys(pageLayout).length === 0) {
        throw invalidPayload(operation, "set_page_layout requires paper_size or margin_*_cm");
      }
      return pageLayout;
    }
    case "set_paragraph_spacing": {
      ...
      if (Object.keys(spacing).length === 0) {
        throw invalidPayload(operation, "set_paragraph_spacing requires before_pt or after_pt");
      }
      return spacing;
    }
    case "set_paragraph_indent": {
      const directIndent = pickPositiveOrZeroNumber(payload.first_line_indent_pt, payload.firstLineIndentPt);
      ...
      if (indentChars === undefined) {
        throw invalidPayload(operation, "set_paragraph_indent requires first_line_indent_pt or first_line_indent_chars");
      }
    ...
    case "set_style_definition": {
      ...
      if (!styleDefinition) {
        throw invalidPayload(operation, "set_style_definition requires style_definition");
      }
    ...
    case "set_numbering_level": {
      ...
      if (!numberingLevel) {
        throw invalidPayload(operation, "set_numbering_level requires numbering_level");
      }
    ...
    case "set_settings_flag": {
      ...
      if (!settings) {
        throw invalidPayload(operation, "set_settings_flag requires settings");
      }
      return { settings };
    }
    case "set_attr": {
      ...
      if (!name) {
        throw invalidPayload(operation, "set_attr requires name");
      }
      return {
    ...
    case "remove_attr": {
      const name = pickNonEmptyString(payload.name);
      if (!name) {
        throw invalidPayload(operation, "remove_attr requires name");
      }
      return {
        ...(pickNonEmptyString(payload.path) ? { path: String(payload.path).trim() } : {}),
        name
    ...
    case "ensure_node": {
      ...
      if (!path || !xmlTag) {
        throw invalidPayload(operation, "ensure_node requires path and xml_tag");
      }
    ...
    case "replace_node_xml": {
      ...
      if (!nodeXml) {
        throw invalidPayload(operation, "replace_node_xml requires node_xml");
      }
      return {
        ...(pickNonEmptyString(payload.path) ? { path: String(payload.path).trim() } : {}),
        node_xml: nodeXml
    ...
    case "split_paragraph": {
      ...
      if (splitOffset === undefined) {
        throw invalidPayload(operation, "split_paragraph requires split_offset");
      }
...
function invalidPayload(operationType: OperationType, message: string): AgentError {
...
function pickPositiveInteger(...values: unknown[]): number | undefined {
  for (const value of values) {
    const parsed = pickPositiveNumber(value);
...
function pickLineSpacingValue(value: unknown): number | { mode: "exact"; pt: number } | undefined {
  ...
  return mode === "exact" && pt !== undefined ? { mode: "exact", pt } : undefined;
}
...
function pickHighlightColor(...values: unknown[]): string | undefined {
  for (const value of values) {
    ...
    const mapped = highlightColorAliases[normalized];
    if (mapped) {
      return mapped;
    }
  }
...
function pickRequiredBoolean(operationType: OperationType, fieldName: string, ...values: unknown[]): boolean {
  for (const value of values) {
...
function isSemanticAlignmentPayload(payload: Record<string, unknown>): boolean {
  return payload.baseline_from_semantic === "body_like_paragraphs" || Array.isArray(payload.sync_fields);
}
