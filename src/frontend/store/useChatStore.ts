...
interface ChatState {
  ...
  turnJobs: Record<string, FrontendJobSnapshot[]>;
  currentSessionId: string | null;
  shouldCreateNewSession: boolean;
  attachedDocuments: Record<string, AttachedDocumentSummary | null>;
...
const defaultSettings: UserSettings = {
  ...
  plannerBaseUrl: 'https://api.deepseek.com/v1',
  plannerApiKey: 'sk-your-api-key-here',
...
function isDraftSession(session: FrontendSession): boolean {
  return session.isDraft === true || isDraftSessionId(session.sessionId);
}

function createDraftSession(): FrontendSession {
  const now = Date.now();
  return {
...
function toSidebarSession(session: SessionDetail): FrontendSession {
  return {
...
function upsertSession(sessions: FrontendSession[], nextSession: FrontendSession): FrontendSession[] {
...
function getDraftSession(sessions: FrontendSession[]): FrontendSession | undefined {
  return sessions.find(isDraftSession);
}

function hidePendingRemoteSessions(
  remoteSessions: FrontendSession[],
  draftSessions: FrontendSession[]
): FrontendSession[] {
...
function combineVisibleSessions(state: ChatState, remoteSessions: FrontendSession[]): FrontendSession[] {
  const drafts = state.sessions.filter(isDraftSession);
  return [...drafts, ...hidePendingRemoteSessions(remoteSessions, drafts)];
}

function getSessionById(state: ChatState, sessionId: string | null): FrontendSession | undefined {
  return state.sessions.find((session) => session.sessionId === sessionId);
}

function resolveCurrentSessionId(state: Pick<ChatState, 'currentSessionId' | 'shouldCreateNewSession'>, sessions: FrontendSession[]): string | null {
...
function chooseNextSessionIdAfterDeletion(sessions: FrontendSession[], deletedSessionId: string): string | null {
...
function createLocalMessage(sessionId: string, content: string): FrontendChatMessage {
  return {
...
function createPendingJob(sessionId: string, tempMessageId: string, steps: ProgressStep[]): FrontendJobSnapshot {
  const now = Date.now();
  return {
...
function upsertJobStep(steps: ProgressStep[], nextStep: ProgressStep): ProgressStep[] {
...
function markLatestRunningStepFailed(steps: ProgressStep[], detail: string): ProgressStep[] {
...
function mergeJobSteps(clientSteps: ProgressStep[], remoteSteps: ProgressStep[]): ProgressStep[] {
...
function replaceTurnJob(
  jobs: FrontendJobSnapshot[],
  jobId: string,
  updater: (job: FrontendJobSnapshot) => FrontendJobSnapshot
): FrontendJobSnapshot[] {
...
function isPlaceholderConfig(modelConfig: {
  chat: { apiKey: string; model: string };
}): boolean {
  const placeholderKeys = new Set(['sk-...', 'sk-your-api-key-here', 'sk-local-gemma4', '']);
  return placeholderKeys.has(modelConfig.chat.apiKey) || modelConfig.chat.model === 'model_name';
}
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  initialize: async () => {
    ...
    try {
      ...
      set({
        ...
        isSettingsOpen: isPlaceholderConfig(modelConfig),
      });
  ...
  sendMessage: async (text) => {
    ...
    const localPreflightSteps: ProgressStep[] = [];
    ...
    const updateJob = (targetSessionId: string, jobId: string, updater: (job: FrontendJobSnapshot) => FrontendJobSnapshot) => {
      set((innerState) => ({
        turnJobs: {
          ...innerState.turnJobs,
    ...
    try {
      ...
      let finalSnapshot: { job: FrontendJobSnapshot; session?: SessionDetail | null } | null = null;
      for (;;) {
        await sleep(800);
        ...
        updateJob(sessionKey, activeJobId, (job) => ({
          ...
          outputPath: snapshot.job.outputPath,
          steps: mergeJobSteps(
